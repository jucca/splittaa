...
type SeededGroup = Doc<"groups"> & { _id: Id<"groups"> };
type SeededExpense = Doc<"expenses"> & { _id: Id<"expenses"> };
  ...
  handler: async (ctx) => {
    if (process.env.ALLOW_DEV_SEED !== "true") {
      throw new ConvexError({
        code: "FORBIDDEN",
        message: "Set ALLOW_DEV_SEED=true in Convex dashboard env to run seed",
      });
    }
...
async function createGroups(
  ...
  return await Promise.all(
    groupIds.map(async (id): Promise<SeededGroup> => {
...
async function createOneOnOneExpenses(
  ...
  return await Promise.all(
    expenseIds.map(async (id): Promise<SeededExpense> => {
...
async function createSettlements(
  ...
  users: Doc<"users">[],
  groups: SeededGroup[],
  oneOnOneExpenses: SeededExpense[],
  groupExpenses: SeededExpense[]
  ...
  const cabExpense = oneOnOneExpenses.find(
    (expense: SeededExpense) => expense.description === "Cab ride to airport"
  );

  const hotelExpense = groupExpenses.find(
    (expense: SeededExpense) => expense.description === "Hotel reservation"
  );

  const coffeeExpense = groupExpenses.find(
    (expense: SeededExpense) => expense.description === "Coffee and snacks"
  );
